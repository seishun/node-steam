...
module.exports = function(username, password, emit) {
  this.handlers = {};
  this.users = {};
  ...
  this.netMsgReceived = function(data) {
    ...
    if (!eMsg) {
      this.emit('debug', "Unknown eMsg: " + rawEMsg + "\nPacket: " + data);
      return;
    }
    ...
    if (eMsg == 'ChannelEncryptRequest' || eMsg == 'ChannelEncryptResult') {
      ...
      // parse body, save header if it's non-empty and we haven't yet
      var headerLength = data.readInt32LE(4);
      this.protoHeader = this.protoHeader || headerLength && data.slice(8, 8 + headerLength);
      body = schema['CMsg' + eMsg].parse(data.slice(8 + headerLength));
      ...
      if (!this.extendedHdr) {
        this.extendedHdr = data.slice(4, 36);
        // we need the steamid separately too
        this.steamID = data.readUInt64LE(24);
      }
      body = data.slice(36);
    }
...
//      this.emit(eMsg, body);
  ...
  this.send = function(eMsg, body) {
    ...
    if (eMsg == 'ChannelEncryptResponse') {
      data = new Buffer(4 + 8 + 8 + body.length);
      data.writeInt32LE(EMsg.toNumber[eMsg], 0);
      data.writeUInt64LE('18446744073709551615', 4); // TODO: check if Steam actually reads these
      data.writeUInt64LE('18446744073709551615', 12);
      body.copy(data, 20);
      
    } else if ('CMsg' + eMsg in schema) {
      // use saved header and serialize message
      var protoHeader = this.protoHeader || schema.CMsgProtoBufHeader.serialize({
        clientSessionid: 0,
