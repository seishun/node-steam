...
handlers[EMsg.ClientFriendMsgIncoming] = function(data) {
  ...
  var message = friendMsg.message.toString();
...
handlers[EMsg.EconTrading_InitiateTradeProposed] = function(data) {
  ...
  this.emit('tradeProposed', tradeProp.tradeRequestId, new SteamID(tradeProp.otherSteamid), tradeProp.otherName);
};
...
handlers[EMsg.EconTrading_InitiateTradeResult] = function(data) {
  ...
  this.emit('tradeResult', tradeResult.tradeRequestId, tradeResult.response, new SteamID(tradeResult.otherSteamid));
};

handlers[EMsg.EconTrading_StartSession] = function(data) {
  var startSess = schema.CMsgTrading_StartSession.parse(data);
  this.emit('sessionStart', new SteamID(startSess.otherSteamid));
};
