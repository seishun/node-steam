...
SteamClient.prototype._netMsgReceived = function(data) {
  ...
  )(header, body, sourceJobID != '18446744073709551615' && function(header, body, callback) {
    if (header.proto)
...
//  var encRequest = schema.MsgChannelEncryptRequest.decode(data);
  ...
  var sessionKey = require('./crypto_helper').generateSessionKey();
...
handlers[EMsg.ClientLogOnResponse] = function(data) {
  ...
  logonResp = Steam._processProto(logonResp);
  
  var buf = new Buffer(4);
  buf.writeUInt32BE(logonResp.public_ip, 0);
  logonResp.public_ip = Array.prototype.join.call(buf, '.');
...
Steam.SteamFriends = require('./handlers/friends');
Steam.SteamGameCoordinator = require('./handlers/game_coordinator');
Steam.SteamTrading = require('./handlers/trading');
Steam.SteamUnifiedMessages = require('./handlers/unified_messages');
Steam.SteamUser = require('./handlers/user');
Steam.CryptoHelper = require('./crypto_helper');
