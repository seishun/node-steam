...
SteamClient.prototype.connect = function (proxy) {
    this.disconnect();
    ...
    if (proxy) {
        socketParams.proxyHost = proxy.host;
        socketParams.proxyPort = proxy.port;
        socketParams.auths = proxy.auth ? proxy.auth : [socks.auth.None()];
    }


    this._connection.connect(socketParams);
};
