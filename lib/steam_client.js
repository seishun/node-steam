...
SteamClient.prototype._send = function(eMsg, body, jobID) {
  var header;
...
SteamClient.prototype._netMsgReceived = function(data) {
  ...
  if (eMsg == EMsg.ChannelEncryptRequest || eMsg == EMsg.ChannelEncryptResult) {
    ...
    jobID = schema.MsgHdr.parse(data).sourceJobID;
  
  } else if (rawEMsg & protoMask) {
    var header = schema.MsgHdrProtoBuf.parse(data);
    ...
    jobID = header.proto.jobidSource;
  
  } else {
    body = data.slice(schema.ExtendedClientMsgHdr.baseSize);
    jobID = schema.ExtendedClientMsgHdr.parse(data).sourceJobID;
  }
  
  if (eMsg in this._handlers) {
    this._handlers[eMsg].call(this, body, jobID);
  } else {
