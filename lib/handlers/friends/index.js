...
SteamFriends.prototype.requestChatHistory = function(steamID) {
  this._client.send({
    msg: EMsg.ClientFSGetFriendMessageHistory,
    proto: {}
  }, new schema.CMsgClientFSGetFriendMessageHistory({
    steamid: steamID
  }).toBuffer());
};
...
handlers[EMsg.ClientFSGetFriendMessageHistoryResponse] = function(data) {
  var history = Steam._processProto(schema.CMsgClientFSGetFriendMessageHistoryResponse.decode(data));
  ...
  (history.messages || []).forEach(function(message) {
    ...
    message.steamid = sid.toString();
    delete message.accountid;
  });
  
  this.emit('messageHistory', history.steamid, history.success, history.messages || []);
};
