...
handlers[EMsg.ClientUDSInviteToGame] = function(data) {
  var msg = Steam.Internal.CMsgClientUDSInviteToGame.decode(data);
  var steamid = SteamID.decode(msg.steam_id_src).toString();
  
  var friend = this.personaStates[steamid];
  if (!friend) {
    return; // no information about the inviting player
  }
  this.emit('gameInvite', steamid, friend.gameid, msg.connect_string);
};
