...
prototype.sendMessage = function(target, message, type) {
  ...
  if (target.accountType == Steam.EAccountType.Individual || target.accountType == Steam.EAccountType.ConsoleUser) {
    ...
    this._client.send({ msg: EMsg.ClientChatMsg }, Buffer.concat([chatMsg, payload.toBuffer()]));
  }
};
...
prototype.leaveChat = function(steamID) {
  ...
  this._client.send({ msg: EMsg.ClientChatMemberInfo }, Buffer.concat([leaveChat, payload.toBuffer()]));
...
handlers[EMsg.ClientPersonaState] = function(data) {
  schema.CMsgClientPersonaState.decode(data).friends.forEach(function(friend) {
    ...
    buf.writeUInt32BE(friend.game_server_ip, 0);
    friend.game_server_ip = Array.prototype.join.call(buf, '.');
...
handlers[EMsg.ClientFriendMsgIncoming] = function(data) {
  ...
  this.emit('message', friendMsg.steamid_from.toString(), message, friendMsg.chat_entry_type);
  this.emit('friendMsg', friendMsg.steamid_from.toString(), message, friendMsg.chat_entry_type);
};
...
handlers[EMsg.ClientFriendMsgEchoToSender] = function(data) {
  ...
  this.emit('friendMsgEchoToSender', friendEchoMsg.steamid_from.toString(), message, friendEchoMsg.chat_entry_type);
};
...
handlers[EMsg.ClientChatInvite] = function(data) {
  ...
  this.emit('chatInvite', toClanID(chatInvite.steam_id_chat), chatInvite.chat_name, chatInvite.steam_id_patron.toString());
};
