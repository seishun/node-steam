...
var prototype = SteamTrading.prototype;

prototype.trade = function(user) {
  this._client.send({ msg: EMsg.EconTrading_InitiateTradeRequest, proto: {} }, new schema.CMsgTrading_InitiateTradeRequest({
    other_steamid: user
  }));
};

prototype.respondToTrade = function(tradeId, acceptTrade) {
  this._client.send({ msg: EMsg.EconTrading_InitiateTradeResponse, proto: {} }, new schema.CMsgTrading_InitiateTradeResponse({
...
prototype.cancelTrade = function(user) {
  this._client.send({ msg: EMsg.EconTrading_CancelTradeRequest, proto: {} }, new schema.CMsgTrading_CancelTradeRequest({
    other_steamid: user
  }));
};
...
handlers[EMsg.EconTrading_InitiateTradeProposed] = function(data) {
  ...
  this.emit('tradeProposed', tradeProp.trade_request_id, tradeProp.other_steamid.toString(), tradeProp.other_name);
};
...
handlers[EMsg.EconTrading_InitiateTradeResult] = function(data) {
  ...
  this.emit('tradeResult', tradeResult.trade_request_id, tradeResult.response, tradeResult.other_steamid.toString());
};
