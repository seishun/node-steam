...
prototype.activateSteamKey = function(key, callback) {
  this._send(EMsg.ClientRegisterKey | protoMask, schema.CMsgClientRegisterKey.serialize({
    key: key
  }), callback);
};
...
handlers[EMsg.ClientLogOnResponse] = function(data) {
  ...
  if (eresult == EResult.OK) {
    ...
    this.loggedOn = true;
    this.vanityUrl = logonResp.vanityUrl;
...
handlers[EMsg.ClientPurchaseResponse] = function(data, callback) {
  if(!callback) {
    return;
  }
  callback(schema.CMsgClientPurchaseResponse.parse(data));
};
