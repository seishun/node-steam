...
handlers[EMsg.ClientFriendsList] = function(data) {
  ...
  this.emit('relationship', list.friends[0].ulfriendid, list.friends[0].efriendrelationship);
};
...
handlers[EMsg.ClientFriendMsgIncoming] = function(data) {
  ...
  this.emit('message', friendMsg.steamidFrom, message, friendMsg.chatMsgType);
  this.emit('friendMsg', friendMsg.steamidFrom, message, friendMsg.chatMsgType);
};
...
handlers[EMsg.ClientChatEnter] = function(data) {
  ...
  var chatRoom = this.chatRooms[chatEnter.steamIdClan] = {};
  while (numObj--) {
...
handlers[EMsg.ClientChatMemberInfo] = function(data) {
  ...
  if (stateChange == Steam.EChatMemberStateChange.Entered) {
    ...
    this.chatRooms[clanID][chatterActedOn] = {
      rank: object.Details,
      permissions: object.Permissions
    };
  } else {
  ...
  this.emit('chatStateChange', stateChange, chatterActedOn, membInfo.steamIdClan, chatterActedBy);
};
